...
keep = c("Species", "AA_30_positive", "AA83", "Order", "Class")
...
```{r, include = FALSE}
v_file_name = "verttraits_05082020.csv"
V_old <- read.csv("verttraits_30072020_continuousforage.csv")
V <- read.csv(v_file_name)
ind = which(names(V)=="Order")
V = V[,-ind]
setdiff(names(V_old), names(V))
setdiff(names(V), names(V_old))
...
keep = c("Species", "AA83", "AA_30_positive")
...
##find out what happens if we set no lower limit on number of trees -- importance over one
```{r}
min_trees = 0
buffer= nrounds*0.33#buffer to make sure there are enough rounds when it comes to making null model
max_trees = nrounds - buffer
